...
function getTanks2(orgid5) {
    ...
    ref.on("value", function(snapshot) {
        ...
        if (tankInfoFirebase.isCelsius == null) {
            ...
            // console.debug("isCelsius: "+ tankInfo.isCelsius);
        }
        ...
        // adding support for up to 16 tanks, but in a different way
        if (tankInfoFirebase.number > 12 && tankInfoFirebase.number <= 16) {
            if (tankInfoFirebase.number == 13 || tankInfoFirebase.number > 13) {
                if (tankInfoFirebase.tank13.temp==null) {
                    tankInfo.tank13.temp = error.a1010;
                } else {}
            } else {}
            if (tankInfoFirebase.number == 14 || tankInfoFirebase.number > 14) {
                if (tankInfoFirebase.tank14.temp==null) {
                    tankInfo.tank14.temp = error.a1010;
                } else {}
            } else {}
            if (tankInfoFirebase.number == 15 || tankInfoFirebase.number > 15) {
                if (tankInfoFirebase.tank15.temp==null) {
                    tankInfo.tank15.temp = error.a1010;
                } else {}
            } else {}
            if (tankInfoFirebase.number == 16) {
                if (tankInfoFirebase.tank16.temp==null) {
                    tankInfo.tank16.temp = error.a1010;
                } else {}
            } else {
                console.error("Math is wrong: getTanks.js");
            }
        }
        ...
        if (tankInfoFirebase.number > 12 && tankInfoFirebase.number <= 16) {
            if (tankInfoFirebase.number == 13 || tankInfoFirebase.number > 13) {
                var tank13 = tankInfo.tank13.temp;
                document.getElementById("tank13").innerHTML =tank13 + tankDegree;
            } else {}
            if (tankInfoFirebase.number == 14 || tankInfoFirebase.number > 14) {
                var tank14 = tankInfo.tank14.temp;
                document.getElementById("tank14").innerHTML =tank14 + tankDegree;
            } else {}
            if (tankInfoFirebase.number == 15 || tankInfoFirebase.number > 15) {
                var tank15 = tankInfo.tank15.temp;
                document.getElementById("tank15").innerHTML =tank15 + tankDegree;
            } else {}
            if (tankInfoFirebase.number == 16) {
                var tank16 = tankInfo.tank16.temp;
                document.getElementById("tank16").innerHTML =tank16 + tankDegree;
            } else {
                console.error("Math is wrong: getTanks.js");
            }
        }
        ...
        // console.debug("isDynamicColor: "+tankInfo.isDynamicColor)
        if (tankInfo.isDynamicColor==true) {
            dynamicColor(tankInfo);
        } else {
...
function getTanksDisabled(orgid3) {
